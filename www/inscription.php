...
<form class="inscription" action="req_inscription.php" method="post" name="inscription">
	...
	method: indique par quel moyen les données vont être envoyées : post ou get
	-->
	
    <!-- qu'y a-t-il d'autre comme possiblité que post pour l'attribut method ? -->
	<!--
	à la place de post on peut utiliser get
	post: données incluses dans le formulaire et donc invisible à l'utilisateur
	get: données rajoutées à l'URL et donc visible par l'utilisateur
	-->
    ...
    <ul>
        ...
        <li>
			...
			<!--
			id: il est unique et on l'utilise pour mettre un style CSS, créer des ancres
			name: utilisé par PHP pour valider un formulaire par exemple $_POST/GET['name']
			-->
            <!-- c'est lequel qui doit être égal à l'attribut for du label ? -->
			<!--
			l'id doit être égal à l'attribut
			-->
            <span class="form_hint">Format attendu "name@something.com"</span>
        </li>
		<li>
            <label for="nom">Nom :</label>
            <input type="text" name="nom" id="nom" placeholder="Entrez un Nom (optionnel)" />
            <!-- non obligatoire -->
		...
		<li>
			...
			<!--
			 Quand le mdp ne respecte pas la casse et quand le mdp est trop court ou trop long
			-->
        ...
        <li>
            ...
            <input required placeholder="Veuillez confirmer le mot de passe" type="password" id="mdp2" required onkeyup="validateMdp2()">
            <!-- ajouter à input l'attribut qui dit que c'est un champs obligatoire -->
            <!-- ajouter à input l'attribut qui donne une indication grisée (placeholder) -->
            ...
            <script>  
                validateMdp2 = function(e) {  
                    ...
                    if ( mdp1 != "" /* est-ce que mdp1 est valide ? */ && mdp1 == mdp2/* est-ce que mdp1 et mdp2 ont la même valeur ? */) {  					
        ...
        <li>  
			...
			<!--
			disabled est utilisé quand on veut empêcher un utilisateur de modifier une valeur dans un formulaire par exemple
			-->
        </li>
		<li>  
